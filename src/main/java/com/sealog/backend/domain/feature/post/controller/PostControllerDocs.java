...
public interface PostControllerDocs {
    ...
    PageResponse<PostResponse.PostItem> getPosts(Pageable pageable);

    @Operation(summary = "유저의 게시글 목록 조회", description = "nickname 기준으로 PUBLISHED 게시글 목록을 조회합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공"),
    })
    PageResponse<PostResponse.PostItem> getUserPosts(
    ...
    PageResponse<PostResponse.PostItem> getPostsByStack(
            @Parameter(description = "사용자 닉네임", example = "테스터") String nickname,
    ...
    PageResponse<PostResponse.PostItem> searchPosts(
    ...
    PageResponse<PostResponse.PostItem> searchPostsByNickname(
            @Parameter(description = "사용자 닉네임", example = "테스터") String nickname,
            @Parameter(description = "검색 키워드", example = "spring") String keyword,
            Pageable pageable
    );
    ...
    PostResponse.PostDetail getPostByNicknameAndSlug(

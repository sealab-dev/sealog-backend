...
public class TestInitConfig {
    ...
    public void init() {
        ...
        // 1. 유저 생성
        testDataFactory.createTestUsers(10, UserRole.USER);
        testDataFactory.createTestUsers(5, UserRole.ADMIN);

        // 2. 테스트 게시글 생성
        User user = userRepository.findById(1L).orElseThrow(); // id = 0 사용자 기준으로 작성
        testDataFactory.createTestPosts(10, PostStatus.PUBLISHED, user);
